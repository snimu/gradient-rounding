...
I'm training LLMs of different sizes on $1$ epoch of wikitext each, always starting with the same random seed for comparability.
After each backward pass (and, therefore, before any update step), I also round the model's gradients to some number of significant digits.

I ablate those significant digits and look at the results.
...
- [Original motivation](#original-motivation)
    ...
    - [Different behaviors are connected more continuously in high- than in low-dimensional space](#different-behaviors-are-connected-more-continuously-in-high--than-in-low-dimensional-space)
...
You might think that this is not surprising; after all, a smaller model has less model capacity, so obviously it cannot learn as much as a larger one. And yes, it does. However, this only explains why the smaller model stops improving after fewer datapoints than the larger modeld does. It does not explain why even early in training, when there is plenty of model capacity left for learning, the larger model is able to extract more information from the training data than the smaller model.
...
Here is my idle speculation (and that's what this is; I didn't really do any research on this, just did the experiments and wrote this down in two days. If you know anything, please let me know @omouamoua on X/Twitter):

> Larger models live in higher-dimensional space. There, different behaviors are connected much more continuously than in lower-dimensional space.
...
### Different behaviors are connected more continuously in high- than in low-dimensional space

The core of this argument is that SGD is local search. And note that the important thing is that it is *local* search.
...
This makes finding that algorithm with local search much easier than if you had to jump there in a single step,
...
If you want to change a small part of the current algorithm, but nothing else,
...
If you have many parameters, on the other hand, you can find all kinds of algorithms that are exactly like the current ones
...
So let's plot only those curves, and startimg from later in the training where the curves are flatter so that the plot scales more nicely:
...
The rounding seems to make no difference at all in training loss.
...
loss curves of one model with the gradient rounded to different significant digits to that
of the same model rounded to $16$ significant digits, for different model sizes.

Why the L2 distance? Because the loss curves are pretty chaotic and outliers can easily be due to randomness, so I want to reduce their impact.
...
Unfortunately, this plot is so dominated by $1$ to $4$ significant digits that we cannot really make out anything in the more detailed parts. So let's talk about those four.

I notice two main things:

1. The performance degradation with fewer and fewer significant digits in the gradient is superlinear. This means that gradient-digit-scaling is very different from the performance degradation with smaller and smaller model sizes, which is sublinear. The same mechanism might still be at work, with just some small differences in the details accounting for the large differences in scaling behavior, but it means that this basically says nothing about the initial hypothesis I've made.
2. With both width and depth, the difference in the performance increases. I believe that this is because the larger models have a much lower loss at $16$ significant gradient digits than the smaller ones, but at $1$ to $4$ significant digits, the model barely learns at all, and so almost all that increase in performance is reflected in a larger L2-distance.

Let's now look at the same plot, but with only $7$ or more gradient digits.
...
1. The L2-distance decreases with increasing scale. Since the effect I spoke of above (of the model with the least gradient rounding benefitting most from scale) doesn't really apply anymore, this must be another effect that we can now see.
2. Looking closely: increasing depth decreases distance consistently.
3. The same cannot be said for width: at low depth, width decreases the distance, but at high depth it increases it.
4. The number of significant digits in the gradient is apparently irrelevant, as long as it exceeds $6$.

What is the reason for points $1$ through $3$?

My first hypothesis is that they are caused by the fact that with depth, training stability increases.
